...
Warning — do not promote ``-depth`` to the primary key.  A tuple like
``(-depth, F + collapse_count, state_key)`` looks dramatically faster (302 vs
3390 expansions on depth-24; 18K vs 182K on depth-28) because the search
degenerates into greedy depth-first descent.  It returns garbage: cost 298
where the optimal is 24, and 18030 where it is 28.  ``F`` (or
``F + collapse_count``) MUST stay the primary key — ``-depth`` is only a
tie-breaker among equal F values, which is what preserves optimality.
