"""Tests for mp-rbfs: Phase 1 frontier construction, Proc semantics, the
shared duplicate-detection table, all three schedulers, and end-to-end
solving."""
...
from algorithms.mp_rbfs import MPRBFS, _Node, _Proc, _build_procs, _run_phase1
from algorithms.mp_rbfs_schedulers import (
    BestFirstScheduler,
...
def test_build_procs_creates_exactly_one_proc_per_live_frontier_node():
    problem = _TreeProblem(branching=4, max_depth=6)
    phase1 = _run_phase1(problem, num_procs=10, tracker=_tracker())
    procs = _build_procs(phase1)
    assert len(procs) == len(phase1.open_heap)
...
def _make_root_proc(problem: SearchProblem, proc_id: int = 0) -> Tuple[_Proc, dict, "itertools.count[int]"]:
    ...
    root = _Node(state=start, key=key, g=0.0, f=problem.heuristic(start), parent=None, action=None)
    best_g = {key: 0.0}
...
def test_two_procs_do_not_touch_each_others_heap():
    ...
    root_a = _Node(state=(0,), key=state_key((0,)), g=1.0, f=1.0, parent=None, action=0)
    root_b = _Node(state=(1,), key=state_key((1,)), g=1.0, f=1.0, parent=None, action=1)
...
def test_shared_best_g_allows_strictly_better_path_and_marks_old_owner_stale():
    ...
    node_b = _Node(state="B", key="B", g=1.0, f=1.0, parent=None, action="toB")
    node_c = _Node(state="C", key="C", g=1.0, f=1.0, parent=None, action="toC")
...
def test_end_to_end_optimal_path_found_despite_worse_path_discovered_first():
    """Full MPRBFS pipeline on the converging graph: round_robin schedules
...
def test_best_first_scheduler_tends_closer_to_optimal_than_others_on_average():
    """Not a strict per-instance guarantee (mp-rbfs is allowed to be
    ...
    for inst in instances:
        for scheduler in gaps:
            algo = MPRBFS(num_procs=6, scheduler=scheduler, seed=0)
