...
The paper's tables and figures are generated by tracked scripts from the
tracked raw results under `imp_resutls/`:

```bash
python -u paper-scripts/make_tables.py --out ignore/Tables
python -u paper-scripts/make_figures.py --out ignore/Figures   # needs matplotlib
paper-scripts/build_paper.sh    # compiles the LaTeX in ignore/ (4-pass + checks)
```

See `paper-scripts/README.md` for the script -> table/figure mapping and
details.
